...
cd $KERNEL_DIR
KERNELSU_DIR=$(find . -mindepth 0 -maxdepth 4 \( -iname "ksu" -o -iname "kernelsu" \) -type d | cut -c3-)
...
if [[ $KSU_ENABLED == "true" ]] && [[ ! -z "$KERNELSU_DIR" ]]; then
    ...
    if [ -d "$KERNELSU_DIR/kernel" ]; then
    	...
    	cd $KERNELSU_DIR/$KERNELSU_DIR && patch -p1 < enable_susfs_for_ksu_auto.patch
    else
   ...
   [[ $KSU_ENABLED == "true" ]]; then
    ...
    echo "CONFIG_KSU=y" >> $DEVICE_DEFCONFIG_FILE
    echo "CONFIG_KSU_SUSFS=y" >> $DEVICE_DEFCONFIG_FILE
    echo "CONFIG_KPROBES=n" >> $DEVICE_DEFCONFIG_FILE # it will conflict with KSU if it's on
...
if [[ $KSU_ENABLED == "false" ]]; then
    cd $KERNEL_DIR && echo "KernelSU Disabled"
    echo "CONFIG_KSU=n" >> $DEVICE_DEFCONFIG_FILE # just in case KSU is left on by default
